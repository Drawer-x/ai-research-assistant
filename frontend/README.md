...
默认通过 Vite 将 `/api` 代理到 `http://127.0.0.1:8000`。联调其他后端时可设置 `VITE_PROXY_TARGET=http://127.0.0.1:8769`；生产构建需要浏览器直连其他后端时设置 `VITE_API_BASE_URL`。论文上传与其他 Axios 请求使用同一 API 基址。
