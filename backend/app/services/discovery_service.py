...
def _mark_imported(db,user_id,papers):
    ...
    sources=db.scalars(select(PaperExternalSource).where(PaperExternalSource.user_id==user_id,PaperExternalSource.provider=="crossref",PaperExternalSource.external_id.in_(ids))).all() if ids else []
...
def search_external_papers(db,user_id,**kwargs):
    result=CrossrefClient().search_works(**kwargs); _mark_imported(db,user_id,result["items"]); return result
def get_external_paper(db,user_id,external_id):
    paper=CrossrefClient().get_work(external_id); _mark_imported(db,user_id,[paper]); return paper
def resolve_local_paper_external_id(db,paper,user_id):
    ...
    return CrossrefClient().resolve_paper_id(paper.title)
def import_external_paper(db:Session,user_id:int,provider:str,external_id:str,paper_snapshot=None):
    if provider!="crossref": raise ValueError("unsupported provider")
    data=paper_snapshot or CrossrefClient().get_work(external_id); external_id=normalize_doi(data["external_id"] or external_id)
    ...
    paper=Paper(user_id=user_id,title=data["title"] or "Untitled",authors=", ".join(a["name"] for a in data["authors"]),year=data["year"],venue=data["venue"],abstract=data["abstract"],pdf_path=f"external://crossref/{external_id}",full_text=None,parse_status="external")
