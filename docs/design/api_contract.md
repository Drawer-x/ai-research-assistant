...
本文档定义 AI 驱动科研文献分析平台 Sprint 1–3 的联调接口。服务基址默认为 `http://127.0.0.1:8769`，交互文档位于 `/docs`。AI 总结、论文问答、多论文对比、关系生成和 Agent 科研规划通过 adapter 调用对应 service；未配置 `ECNU_API_KEY`、远端调用失败、结构化返回非法或 RAG 数据不可用时自动降级为稳定 fallback。生成的关系、AI 历史与计划均持久化并按用户隔离。
