...
All routes require `Authorization: Bearer <JWT>` and return `{code,message,data}`. The frontend calls this backend and never calls Crossref directly. Sprint 4 uses Crossref Public REST API without an API key; `CROSSREF_MAILTO` is optional.

Crossref requests use a descriptive User-Agent, optional `mailto`, bounded rate limiting, retries and in-memory caching. They send no authentication header. User-specific import flags are applied after cache lookup. Rate-limit, timeout, availability, not-found, and invalid-response failures map to the standard API error envelope.
...
Recommendation output is `{"items":[{"paper": ExternalPaper,"score":0..1,"reasons":[],"seed_paper_ids":[],"is_fallback":false}]}`. Relation output is `{"relations":[{"source":"local:1","target":"crossref:10.x/doi","relation_type":"recommended_from","weight":0..1,"description":"...","directed":true,"is_fallback":false}]}`. Missing strings are `null`, lists are `[]`, numbers are `0`, and booleans are `false`. IDs use `local:<paper_id>`, `crossref:<doi>`, and `recommendation:<id>`. Self-loops, invalid endpoints and non-finite/out-of-range weights are invalid. B must be deterministic, must not invent papers, and must not access the database, JWT, or HTTP routes. Crossref provides real candidates while B ranks them; relation fallback emits only persisted, citation, or recommendation evidence.

## Contract for member C

Discovery: `GET /api/discovery/search?query=&year_from=&year_to=&open_access=&page=1&page_size=20`, `GET /api/discovery/papers/{external_id:path}`, `POST /api/discovery/import` with `{"provider":"crossref","external_id":"10.x/doi"}`. External IDs are canonical DOI strings.
